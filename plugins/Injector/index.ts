...
const isKettu = typeof window.kettu !== 'undefined' || typeof window.Kettu !== 'undefined';
...
if (isKettu || isBlueLoader) {
    // Kettu or BlueLoader - use Vendetta-compatible API
    try {
        ...
        const API = window.kettu || window.Kettu || window.blueloader || window.BlueLoader;
        if (API) {
            ...
            console.error('[MessageInjector] Kettu/BlueLoader API not found!', e);
        }
    }
} else if (isVendetta) {
